...
export const SyncAdvancedConfig = React.memo(
  ({ isDark, customClientId, setCustomClientId }: SyncAdvancedConfigProps) => {
        ...
        >
          <span>
            {showAdvanced
              ...
              : '▸ Свой GitHub OAuth Client ID (опционально)'}
          </span>
        </button>
        {showAdvanced && (
          <div className="mt-2 text-xs flex flex-col gap-2 p-3 rounded-xl bg-inherit border border-inherit">
            ...
            <div className="text-[11px] text-[#9aa0a6] space-y-1 pt-1">
              <p className="font-medium text-inherit">Как включить вход по коду (30 сек):</p>
              <a
              ...
              <p>
                3. Обязательно отметьте галочку <b>«Enable Device Flow»</b> ✅
              </p>
              <p>
                4. Скопируйте полученный <b>Client ID</b> и вставьте сюда.
              </p>
            </div>
          </div>
        )}
