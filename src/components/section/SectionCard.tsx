...
export const SectionCard = React.memo(
  ({ section, sectionIndex, totalSections }: SectionCardProps) => {
    const {
      isDark,
      activeMenuId,
      setActiveMenuId,
    ...
    const isSelfGap =
      ...
      (dragOverSectionGap === draggedSectionIndex ||
        dragOverSectionGap === draggedSectionIndex + 1);
    ...
    return (
      ...
      >
        ...
        {showSectionDropIndicatorBefore && <DropIndicator type="horizontal" position="before" />}

        {/* Section Drop Indicator (After / Bottom) */}
        {showSectionDropIndicatorAfter && <DropIndicator type="horizontal" position="after" />}
        ...
        <div
          className="flex flex-wrap gap-y-3 gap-x-2 select-none min-h-28 items-center"
          onDragOver={(e) => onSectionBodyDragOver(e, section.id)}
          onDrop={(e) => onSectionBodyDrop(e, section.id)}
        >
          {section.items.map((item, itIdx) => {
            const isDragging =
              draggedItemCoords?.sectionId === section.id && draggedItemCoords?.itemIndex === itIdx;
            const isTarget =
          ...
          {isDropTargetAtEnd && section.items.length === 0 && (
            <div className="w-28 h-28 rounded-lg border-2 border-dashed border-[#1a73e8] dark:border-[#8ab4f8] bg-[#1a73e8]/10 dark:bg-[#8ab4f8]/10 flex flex-col items-center justify-center pointer-events-none animate-pulse">
              <Icon
