...
export const getSyncSettings = async (): Promise<SyncSettings> => {
  try {
    if (typeof chrome !== 'undefined' && chrome?.storage?.local) {
      const result = await chrome.storage.local.get([CHROME_NTP_SYNC_SETTINGS_KEY]);
...
export const saveSyncSettings = async (settings: SyncSettings): Promise<void> => {
  try {
    if (typeof chrome !== 'undefined' && chrome?.storage?.local) {
      ...
      window.localStorage.setItem(CHROME_NTP_SYNC_SETTINGS_KEY, JSON.stringify(settings));
    }
  } catch (err) {
